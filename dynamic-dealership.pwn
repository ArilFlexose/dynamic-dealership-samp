...
CMD:editdealer(playerid, params[])
{
    ...
    if(sscanf(params, "ds[24]S()[128]", did, type, string))
    {
        ...
        SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} location, point, price, type, stock, restock, reset, owner, delete");
        return 1;
    }
...
DealerRefresh(id)
{
	if(id != -1)
	{
		...
		if(DealerData[id][dealerPosX] != 0 && DealerData[id][dealerPosY] != 0 && DealerData[id][dealerPosZ] != 0 && strcmp(DealerData[id][dealerOwner], "-") || DealerData[id][dealerOwnerId] != 0)
		{
            ...
            DealerData[id][dealerPickup] = CreateDynamicPickup(1581, 23, DealerData[id][dealerPosX], DealerData[id][dealerPosY], DealerData[id][dealerPosZ]);
		}
